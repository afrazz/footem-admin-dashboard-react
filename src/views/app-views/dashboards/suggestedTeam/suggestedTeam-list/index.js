...
const SuggestedTeamsList = () => {
  ...
  const getSuggestedTeams = async (paginationParams = {}, filterParams) => {
    setLoading(true)
    const data = await suggestedTeamService.getSuggestedTeams(
  ...
  const addSuggestedTeam = () => {
    navigate(`/app/dashboards/suggestedTeam/add-suggestedTeam`)
  }

  const deleteRow = async (row) => {
    const resp = await suggestedTeamService.deleteSuggestedTeam(row._id)
  ...
  const handleFilterSubmit = async () => {
    ...
    form
      ...
      .then(async (values) => {
        ...
        getSuggestedTeams({ pagination: resetPagination() }, sendingValues)
      })
      .catch((info) => {
  ...
  const handleClearFilter = async () => {
    ...
    setPagination(resetPagination())
    getSuggestedTeams({ pagination: resetPagination() }, {})
    setFilterEnabled(false)
  }
    ...
    >
      <Row gutter={8} align="bottom">
        <Col md={6} sm={24} xs={24} lg={6}>
          <Form.Item name="search" label="Team Search">
            <Input placeholder="Search" prefix={<SearchOutlined />} />
          </Form.Item>
        </Col>
        <Col md={6} sm={24} xs={24} lg={6}>
          <Form.Item name="league" label="League Search">
            <Input placeholder="Search" prefix={<SearchOutlined />} />
          </Form.Item>
        </Col>
        <Col md={6} sm={24} xs={24} lg={6}>
...
export default SuggestedTeamsList
