...
const ProductList = () => {
  ...
  const addProduct = () => {
    navigate(`/app/dashboards/category/add-category`)
  }

  const viewDetails = (row) => {
    navigate(`/app/dashboards/category/edit-category/${row._id}`)
  }
  const Newtab = (row) => {
    window.open(`/app/dashboards/category/edit-category/${row._id}`)
  }

  const deleteRow = async (row) => {
    const resp = await categoryService.deleteCategory(row._id)
