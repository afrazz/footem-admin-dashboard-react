...
const LeagueList = () => {
  ...
  const getLeagues = async (paginationParams = {}, filterParams) => {
    setLoading(true)
  ...
  const addProduct = () => {
    navigate(`/app/dashboards/league/add-league`)
  }

  const deleteRow = async (row) => {
    const resp = await leagueService.deleteLeague(row._id)
  ...
  const handleFilterSubmit = async () => {
    ...
    form
      ...
      .then(async (values) => {
        ...
        getLeagues({ pagination: resetPagination() }, sendingValues)
      })
      .catch((info) => {
  ...
  const handleClearFilter = async () => {
    ...
    setPagination(resetPagination())
    getLeagues({ pagination: resetPagination() }, {})
    setFilterEnabled(false)
  }
    ...
    >
      <Row gutter={8} align="bottom">
        <Col md={6} sm={24} xs={24} lg={6}>
          <Form.Item name="search" label="League Search">
