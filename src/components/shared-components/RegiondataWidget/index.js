  ...
  const list = Object.entries(data).map(([key, value]) => (
    ...
    >
      <div>
        ...
        />
        <span className="text-gray-light mx-2">
...
export const RegiondataWidget = (props) => {
  const { data, isLoading, mapSource, mapType, title, content, list } = props
  const isMobile = !utils.getBreakPoint(useBreakpoint()).includes('lg')
  return (
    <Card bodyStyle={{ padding: 0 }}>
      <Row>
        <Col xs={24} sm={24} md={24} lg={24} className="border-right">
          {isLoading ? (
